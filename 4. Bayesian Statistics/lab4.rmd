...
```{r}
plot(density(resid(m_wage_iq)))
qqnorm(resid(m_wage_iq))
```
...
```{r log-resid}
plot(density(resid(m_lwage_iq)))
qqnorm(resid(m_lwage_iq))
```
...
```{r}
summary(m_lwage_iq)
0.0088072+qt(c(0.025,0.975), df = 933)*0.0008694
confint(m_lwage_iq)
```
...
```{r stepBIC,cache=TRUE}
lm1 <- lm(lwage ~ ., data = na.omit(wage))
birthwt.step <- stepAIC(lm1, trace = FALSE)
birthwt.step$anova
```
...
```{r}
wage_na_red = na.omit(wage_red)
bma_lwage2 = bas.lm(lwage ~ . -wage, data = wage_na_red,
                   prior = "BIC", 
                   modelprior = uniform())
bma_lwage2
summary(bma_lwage2)
...
```{r graph}
par(mfrow = c(1,2))
coef_lwage2 = coefficients(bma_lwage2)
plot(coef_lwage2, subset = c(8), ask=FALSE)
